...
int main(int argc, char** argv) {
  ...
  double angular_velocity_rad =
      FLAGS_angular_velocity * sad::math::kDEG2RAD;  // 弧度制角速度
  SE3 pose;                                          // TWB表示的位姿
  Vec3d omega(0, 0, angular_velocity_rad);  // 角速度矢量 w（是角轴也是so(3)）
  ...
  while (ui.ShouldQuit() == false) {
    ...
    Vec3d v_world =
        pose.so3() *
        v_body;  // V_w=V_w'=T_b2w'*V_b=R_b2w*V_b, w'原点与b重合，姿态与w重合
    pose.translation() += v_world * dt;

    // 更新自身旋转
    if (FLAGS_use_quaternion) {
      Quatd q = pose.unit_quaternion() *
                Quatd(1, 0.5 * omega[0] * dt, 0.5 * omega[1] * dt,
                      0.5 * omega[2] * dt);  // P29-(2.62) + P31-(2.78)
      q.normalize();
      pose.so3() = SO3(q);
    } else {
      pose.so3() = pose.so3() * SO3::exp(omega * dt);  // P28-(2.55)
    }
